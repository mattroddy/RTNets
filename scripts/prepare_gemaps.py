...
if speed_setting == 0:
    input_gemaps_dir = data_dir+'/gemaps_features_50ms/'
    output_gemaps_dir = data_dir+'/gemaps_features_processed_50ms/'
elif speed_setting == 1:
    input_gemaps_dir = data_dir+'/gemaps_features_10ms/'
    output_gemaps_dir = data_dir+'/gemaps_features_processed_10ms/'
    # The window_size is 50ms long so needs
    # to be shifted back to avoid looking into future.
    shift_back_amount = 4
if not(os.path.exists(output_gemaps_dir)):
    os.mkdir(output_gemaps_dir)
# os.mkdir(output_gemaps_dir)
# annotation_dir = './extracted_annotations/voice_activity/'

csv_files = os.listdir(input_gemaps_dir)
voice_activity_files = [file.split(
    '.')[0]+'.'+file.split('.')[1]+'.csv' for file in csv_files]
skip_normalization_list = []
if not(os.path.exists(output_gemaps_dir)):
    os.mkdir(output_gemaps_dir)
# for feature_set in ['raw', 'znormalized', '/znormalized_pooled']:
for feature_set in ['znormalized']:
    if not(os.path.exists(output_gemaps_dir+'/'+feature_set)):
        os.mkdir(output_gemaps_dir+'/'+feature_set)
frequency_features_list = ['F0semitoneFrom27.5Hz', 'jitterLocal',
                           'F1frequency', 'F1bandwidth',
                           'F2frequency', 'F3frequency']
frequency_mask_list = [0, 0, 0,
                       0, 0, 0]
energy_features_list = ['Loudness', 'shimmerLocaldB', 'HNRdBACF']
energy_mask_list = [0, 0, 0]
spectral_features_list = ['alphaRatio', 'hammarbergIndex',
                          'spectralFlux', 'slope0-500',
                          'slope500-1500', 'F1amplitudeLogRelF0',
...
gemaps_full_feature_list = frequency_features_list + \
    energy_features_list + spectral_features_list
gemaps_feat_name_list = frequency_features_list + \
    energy_features_list + spectral_features_list
...
def loop_func_one(data):
    ...
    target_csv_gemaps = pd.read_csv(
        input_gemaps_dir+target_file, delimiter=',')
    ...
    for feature in gemaps_full_feature_list:
        if feature in skip_normalization_list:
            if speed_setting == 1:
                ...
                temp_dict[feature][:-
                                   shift_back_amount] = target_csv_gemaps[feature][shift_back_amount:]
            else:
                temp_dict[feature] = target_csv_gemaps[feature]
        else:
            if speed_setting == 1:
                temp_dict[feature] = np.zeros(target_csv_gemaps[feature].shape)
                temp_dict[feature][:-shift_back_amount] = preprocessing.scale(
                    target_csv_gemaps[feature])[shift_back_amount:]
            else:
                temp_dict[feature] = preprocessing.scale(
                    target_csv_gemaps[feature])

            gemaps_feat_name_list.append(feature)
            covarep_std_list.append(np.std(target_csv_gemaps[feature], axis=0))
            covarep_mean_list.append(
                np.mean(target_csv_gemaps[feature], axis=0))
    ...
    for feature in gemaps_full_feature_list:
        if feature in skip_normalization_list:
            if speed_setting == 1:
                temp_dict[feature] = np.zeros(target_csv_gemaps[feature].shape)
                temp_dict[feature][:-
                                   shift_back_amount] = target_csv_gemaps[shift_back_amount:]
            else:
                temp_dict[feature] = np.array(target_csv_gemaps[feature])
        else:
            if speed_setting == 1:
                tmp = np.zeros(target_csv_gemaps[feature].shape)
                tmp2 = (np.array(target_csv_gemaps[feature]) - np.array(
                    mean_pd[feature]))/np.array(variance_pd[feature])
                tmp[:-shift_back_amount] = tmp2[shift_back_amount:]
                temp_dict[feature] = tmp
            else:
                temp_dict[feature] = (np.array(
                    target_csv_gemaps[feature]) - np.array(mean_pd[feature]))/np.array(variance_pd[feature])
...
'''
    ...
    numerator_variance = np.sum(np.tile(np.array(
        num_vals)-1, [np.array(std_list).shape[1], 1]).transpose() * np.array(std_list)**2, axis=0)
    pooled_variance = np.sqrt(numerator_variance/(sum(num_vals)-len(num_vals)))

    numerator_mean = np.sum(np.tile(np.array(num_vals), [np.array(
        mean_list).shape[1], 1]).transpose() * np.array(mean_list), axis=0)
    ...
    for target_file, annotation_file in zip(csv_files, voice_activity_files):
        my_data_two.append(
            [target_file, annotation_file, variance_pd, mean_pd])
